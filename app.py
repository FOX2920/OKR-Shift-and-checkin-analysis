...
class OKRAnalysisSystem:
    ...
    def get_filtered_members(self) -> pd.DataFrame:
        """Get filtered members and store in instance"""
    ...
    def load_and_process_data(self, progress_callback=None) -> Optional[pd.DataFrame]:
        """Load and process all OKR data"""
        if not self.checkin_path:
            raise ValueError("Checkin path not set")

        if progress_callback:
            progress_callback("Loading raw data...", 0.1)
        else:
        ...
        # Fetch Checkin Scores from Base Table
        if progress_callback:
            progress_callback("Fetching checkin scores...", 0.3)
        print("  Fetching checkin scores from Base Table...")
        try:
            table_client = TableAPIClient()
            table_scores_map = table_client.get_checkin_scores()
        except Exception as e:
            print(f"Warning: Could not fetch table scores: {e}")
            table_scores_map = {}
            
        checkin_df = DataProcessor.extract_checkin_data(all_checkins, table_scores_map)
        
        account_df = self.api_client.get_filtered_members()

        if progress_callback:
            progress_callback("Merging data...", 0.5)
        else:
    ...
    def get_user_excel_data(self, user_name: str, okr_shifts: List[Dict], period_checkins: List[Dict], alignment_analysis: Dict) -> Dict:
        """Get Excel data statistics for a specific user matches goal_new.py logic"""
        stats = {}
        
        # 1. OKR Shift (Prioritize Monthly if available)
        # Check if monthly data exists in session state (it is set in run_analysis)
        monthly_okr_data = st.session_state.get('monthly_okr_data', [])
        user_shift_entry = next((u for u in monthly_okr_data if u.get('user_name') == user_name), None)
        
        # Fallback to weekly if monthly not found (though monthly is preferred for Excel)
        if not user_shift_entry:
             user_shift_entry = next((u for u in okr_shifts if u.get('user_name') == user_name), None)
             shift_val = user_shift_entry.get('okr_shift', 0) if user_shift_entry else 0
        else:
             shift_val = user_shift_entry.get('okr_shift_monthly', 0)

        # Calculate percentage (target 33.33 points for 100%)
        if shift_val != 0:
        ...
        # 2. Check-ins
        # Find user object to check co_OKR
        user_obj = None
        user_id = None
        if self.user_manager:
            # Find user_id by name
            for uid, u_name in self.user_manager.user_name_map.items():
                if u_name == user_name:
                    user_id = uid
                    user_obj = self.user_manager.users.get(uid)
                    break
        
        stats['has_okrs'] = "Yes" if (user_obj and user_obj.co_OKR == 1) else "No"
        
        # Checkin Score from period_checkins
        user_period_checkin_data = next((u for u in period_checkins if u.get('user_name') == user_name), None)
        count = user_period_checkin_data.get('checkin_count_period', 0) if user_period_checkin_data else 0
        
        checkin_score = min(count * 2, 8)
        stats['checkin_score'] = True
        stats['checkin_score_val'] = checkin_score
        
        # Collab Score (Default checked)
        stats['collab_score'] = True
        
        # 3. Quality Score (From Base Table)
        scores = []
        if self.user_manager and self.user_manager.checkin_df is not None and user_id:
             user_checkins = self.user_manager.checkin_df[
                 self.user_manager.checkin_df['checkin_user_id'].astype(str) == str(user_id)
             ]
             if not user_checkins.empty and 'next_action_score' in user_checkins.columns:
                 scores = [s for s in user_checkins['next_action_score'] if s > 0]
        ...
        else: stats['quality_low'] = True # Default fallback

        # 4. Section II (Hybrid Logic)
        # Use simple map from alignment_analysis? No, app usage requires detailed extraction.
        # Implment extraction logic here
        ...
        if self.final_df is not None and user_id:
            user_goals_df = self.final_df[
                self.final_df['goal_user_id'].astype(str) == str(user_id)
            ].drop_duplicates('goal_id')
        ...
        # Map to buckets
        if align_score >= 4.5: stats['align_direct_2'] = True
        elif align_score >= 3.5: stats['align_direct_1'] = True
        elif align_score >= 2.5: stats['align_indirect_2'] = True
        elif align_score >= 1.5: stats['align_indirect_1'] = True
        else: stats['align_personal'] = True
    ...
    def _calculate_weekly_shift_data(self, user_df: pd.DataFrame, user_name: str, reference_friday: datetime) -> Dict:
        ...
        user_id = str(user_df['goal_user_id'].iloc[0]) if not user_df.empty and 'goal_user_id' in user_df.columns else ''
...
class GoalAPIClient:
    ...
    def get_filtered_members(self) -> pd.DataFrame:
        ...
        if df.empty:
            df = pd.DataFrame(columns=['id', 'name', 'username', 'job', 'email'])
    ...
    def get_account_users(self) -> pd.DataFrame:
        """Get users explicitly restricted to filtered members (nvvanphong)"""
        # User requested to ONLY use group/get, so we redirect this to get_filtered_members
...
class APIClient:
    ...
    def get_filtered_members(self) -> pd.DataFrame:
        ...
        members = response_data.get('group', {}).get('members', [])
        st.toast(f"ℹ️ Debug: Found {len(members)} members in group 'nvvanphong'")
        print(f"DEBUG: Found {len(members)} members in group 'nvvanphong'")
...
class DataProcessor:
    ...
    @staticmethod
    def extract_checkin_data(all_checkins: List[Dict], scores_map: Dict[str, float] = None) -> pd.DataFrame:
        ...
        for checkin in all_checkins:
            try:
                ...
                # Get Score from Table Map
                # Key format: {user_id}_{timestamp}
    ...
    def clean_final_data(df: pd.DataFrame) -> pd.DataFrame:
        ...
        try:
            ...
            # Fill NaN next_action_score with 0
...
class EmailReportGenerator:
    ...
    def __init__(self, goal_access_token: str, account_access_token: str, smtp_server="smtp.gmail.com", smtp_port=587):
    ...
    def _load_users(self, data_store: Dict):
        """Load users data - STRICTLY using filtered members (nvvanphong)"""
        # Change: Use get_filtered_members instead of get_account_users to restrict scope
        users_df = self.api_client.get_filtered_members() 
        if users_df.empty:
            st.error("Failed to load users data (filtered members)")
            return None
    ...
    def _analyze_overall_checkins(self, all_time_df: pd.DataFrame, all_users: List[str]) -> List[Dict]:
        ...
        for user in all_users:
            try:
                ...
                # DEBUG: Print first user's checkin debug info
                if len(last_week_checkins) > 0 and 'debug_shown' not in st.session_state:
                    st.session_state['debug_shown'] = True
                    st.toast(f"ℹ️ Debug Last Week: {monday_last_week.strftime('%d/%m')} - {sunday_last_week.strftime('%d/%m')}")
                    st.toast(f"User {user} has {len(last_week_checkins)} checkins in range. Dates: {[d.strftime('%d/%m') for d in last_week_checkins]}")
...
def _display_goals_pie_chart_and_table(analyzer, members_without_goals: List):
    ...
    goal_data = pd.DataFrame({
        ...
        'Count': [max(0, members_with_goals), len(members_without_goals)]
    })
    
    # DEBUG INFO
    unique_names_count = len(analyzer.filtered_members_df['name'].unique()) if analyzer.filtered_members_df is not None else 0
    st.caption(f"Debug: Total Members (Rows): {total_members}, Unique Names: {unique_names_count}, Missing Goals: {len(members_without_goals)}")
    if members_with_goals < 0:
        st.error(f"⚠️ Logic Error: Members with goals is negative ({members_with_goals}). Total: {total_members}, Missing: {len(members_without_goals)}")
    ...
    if members_without_goals:
        ...
        cols_to_show = ['name', 'username', 'job', 'email']
        available_cols = [c for c in cols_to_show if c in no_goals_df.columns]
        
        st.dataframe(no_goals_df[available_cols], use_container_width=True, height=300)
...
def _display_checkins_pie_chart_and_table(members_with_goals_no_checkins: List):
    ...
    if members_with_goals_no_checkins:
        ...
        # Defensive column selection
        cols_to_show = ['name', 'username', 'job', 'email']
        available_cols = [c for c in cols_to_show if c in goals_no_checkins_df.columns]
        
        st.dataframe(goals_no_checkins_df[available_cols], use_container_width=True, height=300)
...
ACCOUNT_ACCESS_TOKEN=your_account_token_here
    ...
    try:
        ...
        email_generator = EmailReportGenerator(goal_token, account_token)
