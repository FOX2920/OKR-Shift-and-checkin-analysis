    ...
    def extract_checkin_data(all_checkins: List[Dict], scores_map: Dict[str, float] = None) -> pd.DataFrame:
        ...
        for checkin in all_checkins:
            try:
                ...
                # Fallback to AI Eval (which returns 0 now) if not found in table
...
if __name__ == "__main__":
    ...
    if cycles:
        ...
        if df is not None:
            ...
            weekly_shifts = analyzer.calculate_okr_shifts_by_user()
            monthly_shifts = analyzer.calculate_okr_shifts_by_user_monthly() # ADDED
            print(f"Weekly OKR shifts calculated for {len(weekly_shifts)} users")
            print(f"Monthly OKR shifts calculated for {len(monthly_shifts)} users")
...
for user_id, user in analyzer.user_manager.users.items():
    ...
    print(f"  - Priority Score:  {prio_score}")
...
# Get Checkin Data (assuming period checkins are available, else use overall)
# In goal_test, calculate_okr_shifts_by_user returns weekly_shifts.
# We need checkin counts. 
# analyzer.checkin_df is available.
checkin_counts = defaultdict(int)
if analyzer.user_manager.checkin_df is not None and not analyzer.user_manager.checkin_df.empty:
    if 'checkin_user_id' in analyzer.user_manager.checkin_df.columns:
         counts = analyzer.user_manager.checkin_df.groupby('checkin_user_id').size()
         for uid, count in counts.items():
             checkin_counts[str(uid)] = count
    elif 'user_id' in analyzer.user_manager.checkin_df.columns:
         counts = analyzer.user_manager.checkin_df.groupby('user_id').size()
         for uid, count in counts.items():
             checkin_counts[str(uid)] = count
...
for user_id, user in analyzer.user_manager.users.items():
    ...
    # 1. Shift Score (SWITCHED TO MONTHLY)
    # user_shift_entry = next((u for u in weekly_shifts if str(u.get('user_id', '')) == str(user_id)), None)
    user_shift_entry = next((u for u in monthly_shifts if str(u.get('user_id', '')) == str(user_id)), None)

    shift_val = 0
    if user_shift_entry:
        # Use okr_shift_monthly (points)
    ...
    # Store display string for Row 4
    ...
    # Note: In real app, this is count PER PERIOD. Here we use total count loaded.
    ...
    # Quality (From Base Table)
    # 1. Fetch scores if not already done
    if 'table_scores_map' not in locals():
        try:
             # Use the new client from table_client.py
             # from table_client import TableAPIClient # Moved to top
             if 'table_client' not in locals():
                 table_client = TableAPIClient()
             # Use current cycle context if possible, but get_checkin_scores fetches all/paginated. 
             # We might need to filter by time range later if needed.
             table_scores_map = table_client.get_checkin_scores()
        except Exception as e:
             print(f"Error initializing TableAPIClient: {e}")
             table_scores_map = {}
             
    scores = []
    # checkin_df has 'checkin_content' and 'checkin_user_id', 'checkin_since' (timestamp) (need to verify column name)
    # goal_test logic above used 'checkin_df' which has mapped columns.
    # In API get_all_checkins (goal.py), it returns fields map.
    # Let's assume analyzer.user_manager.checkin_df has 'checkin_since' or 'date'?
    # Checking goal_test.py/app.py logic: Checkins df usually has 'checkin_ts' or similar?
    # View file showed 'since' in extraction?
    # Let's try to find 'checkin_since' or 'since' column in checkin_df.
    # If not present, we can't map.
    # I'll check columns if possible or try likely candidates.
    
    # Filter for user first
    user_checkins = pd.DataFrame()
    if analyzer.user_manager.checkin_df is not None and not analyzer.user_manager.checkin_df.empty:
         if 'checkin_user_id' in analyzer.user_manager.checkin_df.columns:
             user_checkins = analyzer.user_manager.checkin_df[analyzer.user_manager.checkin_df['checkin_user_id'].astype(str) == str(user_id)]
    
    if not user_checkins.empty:
        # Assuming we have timestamp column. 
        # In app.py/DataProcessor, 'checkin_since_timestamp' is the raw epoch
        ts_col = 'checkin_since_timestamp'
        if ts_col not in user_checkins.columns and 'checkin_since' in user_checkins.columns: 
             # Fallback if timestamp not found (unlikely if valid)
    ...
    median_q = np.median(scores) if scores else 0 # Default 0 if no scores found in table
    
    # Print for user verification
    print(f"  - User: {user.name} | Next Action Scores: {scores} | Median: {median_q}")

    if median_q >= 4.5: stats['quality_high'] = True
    elif median_q >= 2.5: stats['quality_med'] = True
    elif median_q > 0: stats['quality_low'] = True 
    # If 0 (no data), no box checked? Or assume low? 
    # "Không có hành động rõ ràng" -> 1. 0 matches nothing.
    # Let's fallback to Low if 0? Or leave empty?
    # "lấy các next action score...". 
    # If table has no data, maybe we should fallback or set to 1.
    if not scores: # Fallback to 1 (Low) if missing, logic as per old heuristic default?
         pass # stats['quality_low'] = True ? No, leave unchecked or check low.
         stats['quality_low'] = True


    # 3. Section II (Use Hybrid Logic)
    # Get unique goals for this user (re-use logic from test block above)
    user_col = 'goal_user_id'
    if 'pro_goal_user_id' in analyzer.final_df.columns: user_col = 'pro_goal_user_id'
    goal_id_col = 'goal_id'
    if 'pro_goal_id' in analyzer.final_df.columns: goal_id_col = 'pro_goal_id'
    ...
    excel_users_data.append({
        'name': user.name,
        'stats': stats
    })

# Generate
print(f"Generating Excel for {len(excel_users_data)} users...")
generator = OKRSheetGenerator()
cycle_name = selected_cycle['name'] if selected_cycle else "Current Cycle"
excel_buffer = generator.generate_excel(excel_users_data, cycle_name)

output_filename = "OKR_Analysis_Report.xlsx"
with open(output_filename, "wb") as f:
    f.write(excel_buffer.getvalue())

print(f"✅ Đã xuất file Excel thành công: {output_filename}")
